...
@include('partials.calendar-header', ['view' => $view, 'monday' => $monday, 'monthAnchor' => $monthAnchor, 'routeName' => 'admin.courses.calendar'])
...
@if ($view === 'month')
  @include('partials.calendar-month', [
    ...
    'routeName' => 'admin.courses.calendar',
  ])
@else
  @php
    $startHour = 8;
      ...
      <div class="cell">
        @foreach ($untimed[$key] as $ev)
          ...
          <div class="cal-event-wrap" style="position:relative;">
            <a href="{{ route('admin.courses.edit', $c) }}" class="cal-event notime {{ $ev['cancelled'] ? 'cancelled' : '' }}">
      ...
      <div class="cal-col {{ $d->isToday() ? 'today' : '' }}" style="height: {{ ($endHour - $startHour) * $hourPx }}px;">
        @foreach ($timed[$key] as $ev)
          ...
          <div class="cal-event-wrap" style="top: {{ $ev['top'] }}px; height: {{ $ev['height'] }}px;">
            <a href="{{ route('admin.courses.edit', $c) }}" class="cal-event {{ $ev['cancelled'] ? 'cancelled' : '' }}">
              <div class="t">{{ $c->title }}</div>
    ...
    <div class="cal-mobile-day">
      ...
      @forelse ($dayEvents as $ev)
        ...
        <div class="row {{ $ev['cancelled'] ? 'cancelled' : '' }}">
          <a href="{{ route('admin.courses.edit', $c) }}" style="color:inherit;display:flex;gap:10px;align-items:center;flex:1;">
