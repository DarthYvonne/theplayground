...
<style>
  ...
  .cal-event { display: block; padding: 8px 10px; border-radius: 8px; background: var(--accent-soft); color: var(--text); border-left: 3px solid var(--accent); transition: background 0.1s; }
  .cal-event:hover { background: #dbe6fb; }
  .cal-event .t { font-weight: 700; font-size: 13px; line-height: 1.25; }
    ...
    <div class="cal-col">
      ...
      <div class="cal-col-body">
        @forelse ($byDay[$key] as $c)
          <a href="{{ route('chat.course', $c) }}" class="cal-event">
  ...
  <div class="cal-unscheduled">
    ...
    <div class="cal-unscheduled-list">
      @foreach ($unscheduled as $c)
        <a href="{{ route('chat.course', $c) }}" class="tag muted" style="padding:6px 12px;">{{ $c->title }}</a>
      @endforeach
    </div>
  </div>
@endif
