...
<style>
  ...
  /* Tank cards */
  .float-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 18px; }
  ...
  /* Week nav header inside a card */
  .float-card .weeknav { display: flex; align-items: center; gap: 6px; padding: 10px 12px; border-bottom: 1px solid #f0f2f5; }
  .float-card .weeknav .label { flex: 1; text-align: center; font-weight: 700; font-size: 13px; color: var(--text); }
  .float-card .weeknav button { background: #fff; border: 1px solid var(--border); color: var(--muted); width: 32px; height: 32px; border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; font-size: 13px; }
  .float-card .weeknav button:hover { background: var(--accent-soft); border-color: var(--accent); color: var(--accent); }

  /* Week plan */
  .week-plan { padding: 8px 12px 12px; max-height: 320px; overflow-y: auto; }
  .week-plan .loading { padding: 30px 10px; text-align: center; color: var(--muted); font-size: 13px; }
  .week-plan .day { padding: 8px 0; border-bottom: 1px solid #f4f6f8; }
  .week-plan .day:last-child { border-bottom: none; }
  .week-plan .day .dh { display: flex; align-items: baseline; gap: 6px; margin-bottom: 6px; }
  .week-plan .day .dh .dl { font-weight: 700; font-size: 12px; color: var(--text); }
  .week-plan .day .dh .dl.today { color: var(--accent); }
  .week-plan .day .dh .ds { font-size: 11px; color: var(--muted); }
  .week-plan .day .none { font-size: 12px; color: var(--muted); }
  .week-plan .chips { display: flex; flex-wrap: wrap; gap: 6px; }
  .week-plan .chip { border: 1px solid var(--border); background: #fff; border-radius: 8px; padding: 5px 9px; font: inherit; font-size: 12px; font-weight: 600; color: var(--text); cursor: pointer; transition: background 0.1s, border-color 0.1s; }
  .week-plan .chip:hover { background: var(--accent-soft); border-color: var(--accent); color: var(--accent); }
  .week-plan .chip.mine { background: #ecfdf5; border-color: #34d399; color: #065f46; cursor: default; display: inline-flex; align-items: center; gap: 5px; }
  .week-plan .chip.mine:hover { background: #ecfdf5; border-color: #34d399; color: #065f46; }
  .week-plan .chip.mine i { font-size: 10px; color: #16a34a; }
  ...
  @else
    <div class="float-cards">
      @foreach ($devices as $device)
        ...
        <div class="float-card" data-device-id="{{ $device->id }}" data-week="{{ $weekParam }}">
          <div class="photo">
          ...
          <div class="weeknav">
            <button type="button" class="nav-prev" aria-label="Forrige uge"><i class="fa-solid fa-chevron-left"></i></button>
            <span class="label">…</span>
            <button type="button" class="nav-next" aria-label="Næste uge"><i class="fa-solid fa-chevron-right"></i></button>
          </div>
          <div class="week-plan"><div class="loading">Indlæser…</div></div>
        </div>
      @endforeach
    </div>
  @endif
...
(function () {
  ...
  function renderPlan(card, data) {
    var deviceId = card.dataset.deviceId;
    var plan = card.querySelector('.week-plan');
    card.querySelector('.weeknav .label').textContent = data.week_label;
    card.querySelector('.nav-prev').dataset.week = data.prev;
    card.querySelector('.nav-next').dataset.week = data.next;

    var html = '';
    data.days.forEach(function (day) {
      var bookable = (day.slots || []).filter(function (s) { return s.status === 'free' || s.status === 'mine'; });
      html += '<div class="day"><div class="dh">' +
        '<span class="dl' + (day.is_today ? ' today' : '') + '">' + esc(day.label) + '</span>' +
        '<span class="ds">' + esc(day.short) + '</span></div>';
      if (!day.is_open) {
        ...
        bookable.forEach(function (s) {
          if (s.status === 'mine') {
            html += '<span class="chip mine"><i class="fa-solid fa-circle-check"></i>' + esc(s.start) + '–' + esc(s.end) + '</span>';
          } else {
    ...
    plan.querySelectorAll('.chip[data-slot]').forEach(function (btn) {
      btn.addEventListener('click', function () { book(deviceId, btn.dataset.slot); });
    });
  }
  ...
  function load(card) {
    ...
    var url = AVAIL_URL + '?device_id=' + encodeURIComponent(card.dataset.deviceId) + '&week=' + encodeURIComponent(card.dataset.week);
    fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
      .then(function (r) { if (!r.ok) throw new Error('http ' + r.status); return r.json(); })
      .then(function (data) { renderPlan(card, data); })
  ...
  document.querySelectorAll('.float-card[data-device-id]').forEach(function (card) {
    card.querySelector('.nav-prev').addEventListener('click', function () {
      card.dataset.week = this.dataset.week; load(card);
    });
    card.querySelector('.nav-next').addEventListener('click', function () {
      card.dataset.week = this.dataset.week; load(card);
    });
    load(card);
  });
